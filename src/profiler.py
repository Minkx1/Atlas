...
class ResourceProfiler:
    def __init__(self, sample_interval: float = 0.5):
        ...
        self.metrics: dict[str, StateMetrics] = defaultdict(
            lambda: StateMetrics(cpu_samples=[])
        )
        self._running = False
    ...
    def get_instant_stats() -> tuple[float, float]:
        """Повертає (CPU %, RAM MB) поточного процесу миттєво."""
    ...
    def get_summary(self) -> dict[str, dict[str, float]]:
        """Повертає зведену статистику для аналізу."""
        summary = {}
